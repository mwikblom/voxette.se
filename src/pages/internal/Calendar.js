...
export default withStyles(styles)(
  class InternalCalendar extends Component {
    ...
    componentWillMount() {
      ...
      FirebaseApp.voxette.fetchMembers('', '', '', (members) => {
        if (members) {
          ...
          this.setState({
            members: Object.values(members).filter(
              (x) =>
                x.userData.tags === undefined ||
                !x.userData.tags.includes(Constants.inactive)
            ),
          });
        }
      });
    ...
    handleSelectUser = (e) => {
      ...
      const selectedUser = this.state.members.find(
        (x) => x.userData.memberId === memberId
      ).userData;
      this.setState({
        selectedUser,
      });
    ...
    render() {
      ...
      const fromDateFormatted =
        fromDate && typeof fromDate === 'object'
          ? DateTimeHelper.getFormattedDate(fromDate)
          : '';
      const toDateFormatted =
        toDate && typeof toDate === 'object'
          ? DateTimeHelper.getFormattedDate(toDate)
          : '';
      return (
        <div>
          {!showForm && (
          ...
          {showForm && (
            <CalendarEventForm
              closeFormEvent={(id, e) =>
                this.handleToggleEventForm(false, id, e)
              }
              event={selectedEvent ? selectedEvent.event : undefined}
          ...
          <div className={classes.filterContainer}>
            ...
            {isAdmin && selectedUser && (
              <FormControl className={classes.userSelect}>
                ...
                >
                  <MenuItem value={user.memberId} selected>
                    {user.firstName} {user.lastName}
                  </MenuItem>
                  {members
                    .filter((x) => x.userData.memberId !== user.memberId)
                    .map(({ userData }) => (
                      <MenuItem
                        key={userData.memberId}
                ...
                <div key={i}>
                  <Grid container spacing={24} className={classes.eventGrid}>
                    <CalendarItem
                      ...
                      eventId={eventId}
                      key={i}
                      handleSelectEditEvent={(e, id) =>
                        this.handleSelectEditEvent(e, id)
                      }
                    />
                    {(event.eventData.hasAttendanceCheck === undefined ||
                      event.eventData.hasAttendanceCheck === true) && (
                      <React.Fragment>
                        <AttendanceCheck
                          user={user}
